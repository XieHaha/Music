package com.zyc.doctor.ui.activity.xiaoyu.link;
...
import com.zyc.xylink.utils.BackHandledFragment;
import com.zyc.xylink.utils.BackHandledInterface;
...
public class CallActivity extends FragmentActivity implements BackHandledInterface
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        MeetingBean bean = (MeetingBean)intent.getSerializableExtra(CommonData.KEY_MEETING_BEAN);
        if (bean != null)
        {
            mDialFragment.setMyNumber(bean.getMeetingRoomNumber());
            mDialFragment.setDisplayName(bean.getMeetingRoomName());
            mDialFragment.setMeetingBean(bean);
        }
        else
        {
        }
        ...
        NemoSDK.getInstance().setNemoSDKListener(new NemoSDKListener()
        {
                              ...
                              public void accept(CallState callState) throws Exception
                              {
                                  switch (state)
                                  {
                                      case CONNECTING:
                                          ...
                                          {
                                              manager.beginTransaction()
                                                     .add(R.id.content_frame, mVideoFragment)
                                                     .commitAllowingStateLoss();
                                          }
                                      ...
                                      case DISCONNECTED:
                                          Log.i(TAG,
                                                "CallInfo nemoSDKDidReceiveCall onCallStateChange  is==" +
                                                reason);
                                          if (reason.equals("CANCEL"))
                                          {
                                              Toast.makeText(CallActivity.this, "call canceled",
                                                             Toast.LENGTH_SHORT).show();
                                          }
                                          if (reason.equals("BUSY"))
                                          {
                                              Toast.makeText(CallActivity.this,
                                                             "the side is busy, please call later",
                                                             Toast.LENGTH_SHORT).show();
                                              manager.beginTransaction()
                                                     .hide(mDialFragment)
                                                     .show(mVideoFragment)
                                          ...
                                          {
                                              L.i(TAG, "mVideoFragment reason is==" + reason +
                                          ...
                                          {
                                              L.i(TAG, "mVideoFragment reason is 222==" + reason +
                                                       "==callNumber==" + callNumber);
                                              if (mVideoFragment.isAdded())
                                              {
                ...
                Observable.just(videoInfos)
                          ...
                          .subscribe(new Consumer<List<VideoInfo>>()
                          {
                              ...
                              public void accept(List<VideoInfo> videoInfos)
                              {
                                  ...
                                  Log.i(TAG,
                                        " onVideoDataSourceChange is " + videoInfos.toString());
                              }
                          });
            }

            @Override
            public void onConfMgmtStateChanged(final int callIndex, final String opearation,
                    final boolean isMuteDisable)
            {
            ...
            public void onRosterChange(RosterWrapper roster)
            {
                ...
                if (roster != null)
                {
                    ...
                    {
                        L.i(TAG, "onRosterChange deviceName=" + r.getDeviceName() + ", pid=" +
                                 r.getParticipantId());
                    }
                }
            }
                    ...
                    public void run()
                    {
                        if (videoStatus == VideoStatus.VIDEO_STATUS_NORMAL.getStatus())
                        {
                            Toast.makeText(CallActivity.this, "网络正常", Toast.LENGTH_SHORT).show();
                                 ...
                                 VideoStatus.VIDEO_STATUS_LOW_AS_LOCAL_BW.getStatus())
                        {
                            Toast.makeText(CallActivity.this, "本地网络不稳定", Toast.LENGTH_SHORT).show();
                        }
                        else if (videoStatus ==
                                 VideoStatus.VIDEO_STATUS_LOW_AS_LOCAL_HARDWARE.getStatus())
                        {
                            Toast.makeText(CallActivity.this, "系统忙，视频质量降低", Toast.LENGTH_SHORT)
                                 .show();
                        }
                        ...
                        {
                            Toast.makeText(CallActivity.this, "网络不稳定，请稍候", Toast.LENGTH_SHORT)
                                 .show();
                        }
                        else if (videoStatus ==
                                 VideoStatus.VIDEO_STATUS_LOCAL_WIFI_ISSUE.getStatus())
                        {
                            Toast.makeText(CallActivity.this, "WiFi信号不稳定", Toast.LENGTH_SHORT)
                                 .show();
                        }
                    }
                });
            ...
            @Override
            public void onIMNotification(final int callIndex, final String type,
                    final String values)
            {
                ...
                runOnUiThread(new Runnable()
                {
                    ...
                    public void run()
                    {
                        if ("[]".equals(values))
                        {
                            Toast.makeText(CallActivity.this, R.string.im_notification_ccs_transfer,
                                           Toast.LENGTH_SHORT).show();
                        }
                        else
                        {
                            ...
                            String str = String.format("%s%s%s", getResources().getString(
                                    R.string.queen_top_part), val, getResources().getString(
                                    R.string.queen_bottom_part));
            ...
            {
                Log.i(TAG, "CallInfo nemoSDKDidReceiveCall callActivity is" + name + "==number==" +
                           number + "==callIndex==" + callIndex);
                callNumber = number;
            }

            @Override
            public void onDualStreamStateChange(int callIndex, NemoDualState state, int mode,
                    String reason)
            {
                Log.i(TAG,
                      "nemoSDK onDualStreamStateChange CallActivity is=::" + callIndex + "=state=" +
                      state + "=mode=" + mode + "=reason=" + reason);
            }
        });
    }
    ...
    {
        new AlertDialog.Builder(CallActivity.this).setTitle("来电")
                                                  .setMessage(
                                                          "呼叫人: " + callerName + "\n" + "来电号码： " +
                                                          callerNumber)
                                                  .setNegativeButton("接听",
    ...
    {
        VIDEO_STATUS_NORMAL(0), VIDEO_STATUS_LOW_AS_LOCAL_BW(1), VIDEO_STATUS_LOW_AS_LOCAL_HARDWARE(
            2), VIDEO_STATUS_LOW_AS_REMOTE(3), VIDEO_STATUS_NETWORK_ERROR(
            4), VIDEO_STATUS_LOCAL_WIFI_ISSUE(5);
        private int status;
