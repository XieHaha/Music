...
public class NavigaterPageIndex extends LinearLayout
{
    ...
    public NavigaterPageIndex(Context paramContext,
                              AttributeSet paramAttributeSet) {
        super(paramContext, paramAttributeSet);
        this.mContext = paramContext;
        TypedArray a = mContext.obtainStyledAttributes(paramAttributeSet,
                                                       R.styleable.NavigaterPageIndex);
        paddingIndex = a.getDimensionPixelSize(
                R.styleable.NavigaterPageIndex_indexPadding, 0);
        paddingBottom = a.getDimensionPixelSize(
                R.styleable.NavigaterPageIndex_paddingBottom, 0);
        a.recycle();
    }
    ...
    private void addPageIndex(int currentPageIndex) {
        ...
        for (int j = 0; j < mPageCount; j++) {
            if (currentPageIndex == j) {
                pageImageView = (ImageView) mViewArray.get(Integer
                        .valueOf(j), null);

                if (pageImageView == null) {
                    // add imageview to show dot
                    pageImageView = new ImageView(mContext);
                    pageImageView.setPadding(paddingIndex, 0, paddingIndex,
                            paddingBottom);
