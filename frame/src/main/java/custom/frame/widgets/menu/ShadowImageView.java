package custom.frame.widgets.menu;
...
public class ShadowImageView extends FrameLayout
{
            ...
            @AttrRes int defStyleAttr)
    {
        ...
        try
        {
            ...
            mImgId = a.getResourceId(R.styleable.ShadowImageView_imgSrc, -1);
        }
        catch (Exception e)
        {
            e.printStackTrace();
    ...
    {
        Bitmap bitmap = createShadowBitmap(w, h, mShadowRadius, mShadowOffsetX, mShadowOffsetY,
                                           Color.TRANSPARENT);
    ...
    private Bitmap createShadowBitmap(int shadowWidth, int shadowHeight, float shadowRadius,
            float shadowOffsetX, float shadowOffsetY, int fillColor)
    {
        ...
        shadowPaint.setShadowLayer(shadowRadius, shadowOffsetX, shadowOffsetY,
                                   Color.parseColor(DEFAULT_SHADOW_COLOR));
